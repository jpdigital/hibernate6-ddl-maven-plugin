...
for (def dialect : dialects) {
    ...
    def tablesThatShouldNotExist = ["companies"];
    for (def table : tablesThatShouldNotExist) {
        if (ddlScript.toLowerCase().contains(
            String.format("create table %s", table)
        )) {
            throw new RuntimeException(
                String.format(
                    "DDL script contains a create statement for table %s, but "
                        + "the package containing this entity was not included "
                        + "in the configuration and should not have been "
                        + "processed by the Mojo.",
                    table
                )
            );
        }
